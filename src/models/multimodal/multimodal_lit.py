...
from .multimodal import MultiModalModel, LanguageModel, \
    calculate_attn_reg_loss
from .utils import get_entropy
from .textgen_eval import evaluate as textgen_eval
from .multimodal_data_module import \
...
class MultiModalLitModel(pl.LightningModule):
    ...
    def __init__(self, vision_encoder, text_encoder, args):
        ...
        self.save_hyperparameters(ignore=["vision_encoder", "text_encoder"]) #* modified for warning
    ...
    @staticmethod
    def load_model(model_name="cvcl-resnext", seed=0, device='cuda:0'):
        """Load pre-trained CVCL model from HuggingFace Hub"""
        model_configs = {
            "cvcl-resnext": "cvcl_s_dino_resnext50_embedding",
            "cvcl-vit": "cvcl_s_dino_vit_embedding"
        }

        checkpoint_name = model_configs.get(model_name)
        if not checkpoint_name:
            print(f"Model name not found {model_name}, loading default cvcl-resnext")
            checkpoint_name = model_configs["cvcl-resnext"]

        if seed != 0 and model_name.split("-")[-1] != "vit":
            file_name = f"{checkpoint_name}_seed_{seed}.ckpt"
        else:
            file_name = f"{checkpoint_name}.ckpt"

        checkpoint = hf_hub_download(repo_id=f"wkvong/{checkpoint_name}", filename=file_name)
        print(f"Loading checkpoint from {checkpoint}")

        # Remap old imports from checkpoint to new codebase structure
        import sys
        sys.modules['multimodal'] = sys.modules['models.multimodal']
        model = MultiModalLitModel.load_from_checkpoint(checkpoint_path=checkpoint, device=device)
